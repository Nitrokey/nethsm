...
let unquote s =
  let l = String.length s in
  if is_quoted s (* if its not a JSON object *) then String.sub s 1 (l - 2)
  else s

let escape s =
  if is_quoted s (* if its not a JSON object *) then s
  else
    let s' = Str.global_replace (Str.regexp_string "\"") "\\\"" s in
    "\"" ^ s' ^ "\""

let get_endpoints meta =
  Ezjsonm.get_dict meta
  |> List.partition (fun (key, _v) -> CCString.prefix ~pre:"/" key)

let get_meth meth meta =
  (* e.g. met is "get", "put", "post" *)
...
let example_of_type json =
  ...
  | Some (`String ref) -> (
      ...
      try Ezjsonm.find type_json [ "example" ]
      with Not_found -> failwith ("Couldn't find example for type " ^ ref))
...
let prepare_setup _meth _path _cmd (state, role, _req) =
  ...
  let provision =
    "NETHSM_URL=\"https://localhost:8443/api\" ../../provision_test.sh"
  in
  let lock =
  ...
  let add_user role =
    ...
    let header = auth_header (passphrase "Administrator") in
    let data =
      Printf.sprintf "{ realName: %S , role: %S , passphrase: %S }" user role
        pass
    in
    cmd ("/users/" ^ user) "PUT"
    ^ header ^ "-H \"Content-Type: application/json\" --data " ^ escape data
  in
  let prepare_role =
    match role with
...
type req_data = {
  ...
  auth_header : string;
  (* command to add authentication headers for that role *)
...
let make_req_data req meth =
  ...
  let states_and_data_for_mediatype =
    match meth with
    | "get" -> [ { states; role; auth_header; substs } ]
    | "post" | "put" ->
        List.map
          (fun d -> { states; role; auth_header = auth_header ^ d; substs })
          (make_post_data req)
    | m ->
        Printf.printf "Error: Method %s not allowed\n" m;
        assert false
  in
  let unroll_states { states; role; auth_header; substs } =
    let other_states =
      List.filter (fun x -> not @@ List.mem x states) all_states
    in
    List.append
      (List.map
         (fun s -> { test_res = `Neg; state = s; role; auth_header; substs })
         other_states)
      (List.map
         (fun s -> { test_res = `Pos; state = s; role; auth_header; substs })
         states)
  in
...
let make_resp_data raml_meth =
  let response_codes =
    Ezjsonm.get_dict @@ Ezjsonm.find raml_meth [ "responses" ]
  in
  let get_example (code, meta) =
    match code with
    | "200" -> (
...
let tests_for_states meth path cmd (response_code, response_body)
  ...
  let test_cmd =
    Printf.sprintf "%s %s  -D %sheaders.out -o body.out \n\n" cmd' req resp_code
  in
  ...
  if test_res = `Pos then (
    ...
    List.iter
      (fun (match', _) ->
        if has_match cmd match' then
          ...
          let wrong_key_cmd =
            Printf.sprintf
              "%s %s  -D 404_wrong_%s_headers.out -o /dev/null \n\n" wrong_key
              req match'
          in
    ...
    let wrong_meth =
      Str.global_replace (Str.regexp_string {|GET|}) other_method cmd'
    in
    let wrong_meth' =
      Str.global_replace (Str.regexp_string {|PUT|}) other_method wrong_meth
    in
    let wrong_meth'' =
      Str.global_replace (Str.regexp_string {|POST|}) other_method wrong_meth'
    in
    if cmd' <> wrong_meth'' then
      let wrong_meth_cmd =
        Printf.sprintf "%s %s  -D 501_wrong_meth_headers.out -o /dev/null \n\n"
          wrong_meth'' req
      in
  ...
  let shutdown_cmd =
    if path = "/system/shutdown" then "sleep 1; exit 1"
    else
  ...
  if test_res = `Pos then (
    ...
    let reply =
      let status =
        Cohttp.Code.(reason_phrase_of_code (int_of_string response_code))
      in
