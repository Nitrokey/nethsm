...
= struct
  ...
  module Stack = Tcpip_stack_direct.MakeV4V6(Time)(Rng)(Net)(Eth)(Arp)(Ip)(Icmp)(Udp)(Tcp)
  ...
  let connect net eth arp =
    Lwt.return ({net; eth; arp}, ref Unconfigured)

  let setup ({net; eth; arp}, state) ?gateway cidr =
    match !state with
    | Ready _ -> Fmt.invalid_arg "Stack is already configured. Call disconnect first."
    | Unconfigured ->
