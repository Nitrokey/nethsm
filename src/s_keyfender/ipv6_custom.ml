...
module Make (N : Mirage_net.S)
            (E : Ethernet.S)
            (NDP : module type of Ndpv6) = struct
  ...
  let start_ticking t u =
    let rec loop u =
      ...
      let ctx, outs = NDP.tick ~now t.ctx in
      t.ctx <- ctx;
      let u = match u, NDP.get_ip t.ctx with
        | None, _ | _, [] -> u
  ...
  let write t ?fragment:_ ?ttl:_ ?src dst proto ?(size = 0) headerf bufs =
    ...
    in
    let ctx, outs = NDP.send ~now t.ctx ?src dst proto size' fillf in
    t.ctx <- ctx;
  ...
  let src t ~dst = NDP.select_source t.ctx dst

  let get_ip t =
    NDP.get_ip t.ctx

  let configured_ips t =
    NDP.configured_ips t.ctx
  ...
  let connect ?(no_init = false) ?(handle_ra = true) ?cidr ?gateway netif ethif =
    ...
    let now = Mirage_mtime.elapsed_ns () in
    let ctx, outs = NDP.local ~handle_ra ~now (E.mac ethif) in
    let ctx, outs = match cidr with
      | None -> ctx, outs
      | Some p ->
        let ctx, outs' = NDP.add_ip ~now ctx p in
        let ctx = NDP.add_prefix ~now ctx (Ipaddr.V6.Prefix.prefix p) in
        ctx, outs @ outs'
    in
    let ctx = match gateway with
      | None -> ctx
      | Some ip -> NDP.add_routers ~now ctx [ip]
    in
    let t = {ctx; ethif} in
    if no_init then
      Lwt.return t
