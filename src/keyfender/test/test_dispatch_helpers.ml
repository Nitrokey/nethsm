...
let ( let* ) v f = match v with None -> () | Some v -> f v

module Expect = struct
  type cohttp_response =
    Cohttp.Code.status_code * Cohttp.Header.t * Cohttp_lwt.Body.t * string list

  type 'a response = 'a * cohttp_response option

  let status_fmt f s = Fmt.string f (Cohttp.Code.string_of_status s)
  let status = Alcotest.testable (Fmt.option status_fmt) (Option.equal ( = ))
  ...
  let code c (hsm, response) =
    Alcotest.(check status)
      "Response code" (Some c)
      (status_of_response response);
    match response with Some (s, _, _, _) when s = c -> Some hsm | _ -> None
