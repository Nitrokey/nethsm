...
let nonempty ~name s =
  guard (String.length s > 0) (Printf.sprintf "JSON field %s is empty." name)

let valid_passphrase ~name s =
  guard
    (10 <= String.length s && String.length s <= 200)
    (Printf.sprintf "passphrase %s is not >= 10 and <= 200 characters" name)

let to_ocaml parse json =
  Rresult.R.reword_error (fun m ->
      Printf.sprintf "Invalid data for JSON schema: %s." m)
  @@ parse json
...
let decode_subject json =
  ...
  nonempty ~name:"commonName" subject.commonName >>| fun () -> subject
...
let decode_time s =
  ...
  Rresult.R.reword_error
    (function
      | `RFC3339 ((start, stop), e) ->
          Fmt.str "Failed to decode timestamp: %a at position %d to %d."
            Ptime.pp_rfc3339_error e start stop)
    (Ptime.of_rfc3339 s)
  >>= fun (time, off, _) ->
...
let is_unattended_boot_of_yojson content =
  let parse = function
    | `Assoc [ ("status", `String r) ] ->
        if r = "on" then Ok true
        else if r = "off" then Ok false
        else Error "Invalid status data, expected 'on' or 'off'."
    | _ ->
        Error
          "Invalid status data, expected a dictionary with one entry 'status'."
  in
  ...
  let of_yojson = function
    | `List ms ->
        List.fold_left
          (fun acc m ->
            acc >>= fun acc ->
            mechanism_of_yojson m >>| fun m -> m :: acc)
          (Ok []) ms
        >>| fun ms -> of_list ms
...
let mechanisms_of_string m =
  List.fold_left
    (fun acc r ->
      acc >>= fun acc ->
      mechanism_of_yojson (`String r) >>| fun m -> m :: acc)
    (Ok [])
    (Astring.String.cuts ~sep:"," m)
  >>| fun ms -> MS.of_list ms
...
let tagset_of_string m = Astring.String.cuts ~sep:"," m |> TagSet.of_list
...
let valid_id id =
  guard (String.length id >= 1) "ID cannot be shorter than 1 character."
  >>= fun () ->
...
let decode_generate_key_req s =
  ...
  guard
    ...
    "Mechanism does not match key type"
  >>= fun () ->
